...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">Threads SNS 自動運用ダッシュボード</h2>
    ...
    @unless ($threadsConfigured && $generatorConfigured)
        <div class="alert alert-warning">
            ...
            <ul class="mb-1">
                ...
                @endunless
                @unless ($threadsConfigured)
                    <li>Threads API の認証情報が未設定です（自動投稿・フォロワー取得に必要）。</li>
                @endunless
            </ul>
            設定手順は <code>docs/threads-automation-setup.md</code> を参照してください。
        </div>
    @endunless
                ...
                <div class="card-body">
                    ...
                    @else
                        <p class="text-muted mb-0">
                            ...
                            <code>php artisan threads:snapshot-followers</code> で自動記録できます。
                        </p>
                    @endif
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('threads.followers.store') }}">
                        @csrf
        ...
        <div class="col-md-6 mb-3 mb-md-0">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('threads.generate') }}" class="form-inline">
                        @csrf
        ...
        <div class="col-md-6">
            <div class="card h-100">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('threads.posts.store') }}">
                        @csrf
                        <div class="form-group">
                            <textarea name="body" class="form-control" rows="2" maxlength="500"
                                      placeholder="投稿本文（500文字まで）" required></textarea>
                        </div>
            ...
            @forelse ($drafts as $post)
                <div class="border rounded p-2 mb-2">
                    ...
                    <div class="d-flex flex-wrap align-items-start mt-2">
                        <form method="POST" action="{{ route('threads.posts.schedule', $post) }}"
                        ...
                        <form method="POST" action="{{ route('threads.posts.publish', $post) }}" class="mr-2 mb-1">
                            @csrf
                            <button class="btn btn-sm btn-success" {{ $threadsConfigured ? '' : 'disabled' }}>
                                今すぐ投稿
                            </button>
                        </form>
                        <form method="POST" action="{{ route('threads.posts.destroy', $post) }}" class="mb-1">
                            @csrf
                            @method('DELETE')
        ...
        <div class="card-body">
            ...
            @forelse ($scheduled as $post)
                <div class="border rounded p-2 mb-2">
                    ...
                    <div class="mt-2">
                        <form method="POST" action="{{ route('threads.posts.publish', $post) }}" class="d-inline">
                            @csrf
                            <button class="btn btn-sm btn-success" {{ $threadsConfigured ? '' : 'disabled' }}>
                                今すぐ投稿
                            </button>
                        </form>
                        <form method="POST" action="{{ route('threads.posts.destroy', $post) }}" class="d-inline">
                            @csrf
                            @method('DELETE')
                ...
                <div class="border rounded p-2 mb-2">
                    ...
                    @if ($post->status === 'posted')
                        <small class="text-success">
                            ...
                            @if ($post->permalink)
                                · <a href="{{ $post->permalink }}" target="_blank" rel="noopener">投稿を表示</a>
                            @endif
                        </small>
                    @else
